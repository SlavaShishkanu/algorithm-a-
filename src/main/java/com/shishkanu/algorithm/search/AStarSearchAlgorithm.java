...
/**
 * Main implementation of A star search algorithm
 * 
 * 
...
public class AStarSearchAlgorithm {
    ...
    public List<Point> getPath(final GraphInterface grid, final Point start, final Point goal) {
        final SearchResult searchResult = getResult(grid, start, goal);
     ...
     * {@code Map<Point, Point>} cameFrom -  {@code cameFrom.get(givenPoint)}  gives parent of givenPoint </br>
     * {@code Map<Point, Float>} costSoFar - stores cost of current point that consist of:</br>
     *  cost of movement from start to this point
     *  + cost calculated by heuristic function {@link #heuristic(Point, Point, Point, boolean)}
     * 
     * @param grid
     * @param start
     * @param goal
        ...
        while (!frontier.isEmpty()) {
            ...
            for (final Point nextPoint : grid.getNeighbours(currentPoint)) {
                ...
                if (!costSoFar.containsKey(nextPoint) || newCost < costSoFar.get(nextPoint)) {
                    ...
                    final float priority = newCost + heuristic(start, nextPoint, goal, grid.isDiagonalMovementAllowed());
     ...
     *  read about heuristic on http://theory.stanford.edu/
     *  </a>
     *  
     * Also check cost function {@link Graph#getCost(Point, Point)}, maybe it should be changed
     * or merged into {@link Graph#getNeighbours(Point)} as it called
     * 
...
//          Diagonal distance. Use in case diagonal movement.
...
//          Manhattan distance.Should be used in case diagonal movement disabled.
